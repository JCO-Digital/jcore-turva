...
export default function CleanCspModal({ onClose, onClean }) {
	...
	const isCovered = (subSource, superSource) => {
		if (subSource === superSource) return true;
		if (superSource === "*") return true;
		...
		// Scheme check
		if (sup.scheme && sub.scheme && sup.scheme !== sub.scheme) return false;
		if (sup.scheme && !sub.scheme) return false;
		...
		// Path check
		if (sup.path) {
			if (!sub.path.startsWith(sup.path)) return false;
		}
	...
	const redundancies = useMemo(() => {
		...
		const grouped = sources.reduce((acc, s) => {
			if (!acc[s.directive]) acc[s.directive] = [];
		...
		result.forEach((r) => {
			if (seenRedundant.has(r.redundant.id)) return;
	...
	const handleClean = async () => {
		...
		try {
			...
			alert(__("Cleanup failed.", "jcore-turva"));
		} finally {
			setIsCleaning(false);
		}
	};
	...
	return (
		...
		>
			{sources === null ? (
				...
				<p>{__("No redundant records found.", "jcore-turva")}</p>
			) : (
				<>
					<p>
						{__(
						...
						{redundancies.map((r) => (
							...
							>
								<CheckboxControl
									...
									checked={selectedIds.includes(r.redundant.id)}
									onChange={() => toggleSelection(r.redundant.id)}
								/>
							</div>
						))}
					</div>
			...
			<div className="jcore-turva__modal-actions">
				<Button variant="secondary" onClick={onClose} disabled={isCleaning}>
				...
				<Button
					...
					onClick={handleClean}
					isBusy={isCleaning}
					disabled={isCleaning || sources === null || selectedIds.length === 0}
				>
