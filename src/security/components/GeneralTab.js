...
export default function GeneralTab() {
	...
	useEffect( () => {
		apiFetch( { path: '/jcore-turva/v1/settings' } )
			.then( setSettings )
			.catch( () =>
				setError( __( 'Failed to load settings.', 'jcore-turva' ) )
			);
	}, [] );
				...
				>
					<PanelRow>
						<SelectControl
							...
							value={ settings.csp_mode ?? 'disabled' }
							options={ CSP_MODE_OPTIONS }
							onChange={ ( v ) => update( 'csp_mode', v ) }
						/>
					</PanelRow>
					{ ( settings.csp_mode ?? 'disabled' ) === 'enabled' && (
						<PanelRow>
							<ToggleControl
								__nextHasNoMarginBottom
