...
const REFERRER_OPTIONS = [
	...
	{
		value: 'unsafe-url',
		label: __( 'Unsafe URL — sends full URL everywhere (not recommended)', 'jcore-turva' ),
	},
];
	...
	return (
		<div className="jcore-turva__tab-content">
					...
					{ settings.hsts && (
						<PanelRow>
							<TextControl
								...
								help={ __( 'How long browsers should remember to use HTTPS. Default: 31536000 (1 year).', 'jcore-turva' ) }
								type="number"
								min="0"
								value={ settings.hsts_max_age ?? 31536000 }
								onChange={ ( v ) =>
									update( 'hsts_max_age', parseInt( v, 10 ) || 0 )
								}
							/>
						</PanelRow>
					) }
				...
				>
					<PanelRow>
						<ToggleControl
							...
							help={ __(
								'Sends Content-Security-Policy-Report-Only instead of enforcing CSP. Reports violations without blocking anything. Only active for administrators.',
								'jcore-turva'
							) }
			...
			<div className="jcore-turva__actions">
				...
				{ error && (
					<span className="jcore-turva__error">{ error }</span>
				) }
			</div>
		</div>
	);
