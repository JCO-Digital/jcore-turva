	...
	const handleSubmit = async () => {
		...
		try {
			...
			const sourceExists = existingSources.some(
				(s) => s.directive === targetDirective && s.source === trimmedSource,
			);
	...
	return (
		...
		>
			<div className="jcore-turva__review-form">
				...
				{(() => {
					try {
						const originalUri = normalizeBlockedUri(report.blocked_uri);
		...
		(silent = false) => {
			...
			apiFetch({
				...
				.then((newData) => {
					...
					const prevIds = reportsRef.current.map((r) => r.id).join(",");
				...
				.catch(() => {
					if (!silent) {
						setError(__("Failed to load reports.", "jcore-turva"));
					}
				});
		},
		[statusFilter],
					...
					{statusFilter === "new" && reports?.length > 0 && (
						<>
							{reports.some((r) => r.processed) && (
								<Button variant="secondary" onClick={handleArchiveProcessed}>
								...
								<td>
									<div className="jcore-turva__report-main">
										...
										<div className="jcore-turva__report-details">
											<span>
												{__("In document:", "jcore-turva")}{" "}
												<code title={report.document_uri}>
													{report.document_uri}
												</code>
											</span>
										...
										<div className="jcore-turva__report-meta">
											...
											<span className="jcore-turva__meta-sep">|</span>
											<span>
												{__("Last seen:", "jcore-turva")}{" "}
												{formatDate(report.last_seen)}
											</span>
										</div>
									</div>
								</td>
